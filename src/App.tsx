...
export default function App() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAuthError(error.message || 'Giriş yapılırken hata oluştu.');
    }
  };
  ...
  if (!session) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-slate-800 border border-slate-700 rounded-2xl max-w-sm w-full p-6 shadow-2xl space-y-6">
          <div className="text-center space-y-2">
            ...
            <p className="text-xs text-slate-400">Çekim & Müşteri CRM Yönetim Paneli</p>
          </div>
          ...
          <form onSubmit={handleAuth} className="space-y-4 text-xs">
            ...
            <div>
              <label className="block font-medium text-slate-300 mb-1">E-posta Adresi</label>
              <input
                type="email"
                required
                value={email}
            ...
            <div>
              <label className="block font-medium text-slate-300 mb-1">Şifre</label>
              <input
                type="password"
                required
                value={password}
          ...
          <div className="text-center pt-2 border-t border-slate-700/60">
            ...
            >
              {authMode === 'login' ? 'Hesabınız yok mu? Yeni Hesap Oluştur' : 'Zaten hesabınız var mı? Giriş Yap'}
            </button>
          </div>
        </div>
      </div>
...
export default function App() {
  ...
  // 2. GİRİŞ / KAYIT EKRANI (Oturum Yoksa)
  if (!session) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-slate-800 border border-slate-700 rounded-2xl max-w-sm w-full p-6 shadow-2xl space-y-6">
  ...
  // 3. ANA UYGULAMA PANELİ (Oturum Açılmışsa)
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100">
      {/* Header */}
      <header className="bg-slate-800 border-b border-slate-700 sticky top-0 z-10">
      ...
      {/* MODAL 2: Çekim Ekleme / Düzenleme */}
      {isShootModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-slate-800 border border-slate-700 rounded-xl max-w-md w-full p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-5">
...
export default function App() {
  ...
  const handleShootSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Hatanın Çözümü: shoots tablosuna avatar_url GÖNDERİLMİYOR.
      const shootPayload = {
        client_id: finalClientId,
        client_name: formData.client_name,
        client_phone: formData.client_phone,
