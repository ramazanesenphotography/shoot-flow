...
export default function App() {
  ...
  const handleShootSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Sadece yeni bir dosya seçildiyse yükle
      if (avatarFile) {
        const uploadedUrl = await uploadAvatar(avatarFile);
        if (uploadedUrl) avatarUrl = uploadedUrl;
      }
      ...
      // 1. Yeni müşteri profiliyse `clients` tablosuna kaydet
      if (selectedClientId === 'new' && formData.client_name) {
        ...
        // Var olan müşteriye SADECE YENİ DOSYA YÜKLENDİYSE resim güncelle
      ...
      // 2. Çekim verisi paketle (Buradan avatar_url SİLİNDİ - Hata veren kısım burasıydı)
      const shootPayload = {
      ...
      alert('İşlem gerçekleştirilemedi. Konsol detayına bakabilirsiniz.');
    }
  };
