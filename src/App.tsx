      ...
      <header className="bg-slate-800 border-b border-slate-700 sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap gap-3 justify-between items-center">
          ...
          <div className="flex items-center bg-slate-900 p-1 rounded-xl border border-slate-700 overflow-x-auto max-w-full">
            <button onClick={() => setActiveTab('dashboard')} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition whitespace-nowrap ${activeTab === 'dashboard' ? 'bg-indigo-600 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">Your daily photography workflow at a glance.</p>
            </div>
            <TodayPanel shoots={shoots} onOpenCalendar={() => setActiveTab('shoots')} />
          </div>
        )}
