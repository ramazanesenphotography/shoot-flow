...
export default function App() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAuthError(error.message || 'İşlem gerçekleştirilemedi.');
    }
  };
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-slate-800 border border-slate-700 rounded-2xl max-w-sm w-full p-6 shadow-2xl space-y-5">
          <div className="text-center space-y-2">
            ...
            <p className="text-xs text-slate-400">Çekim & Müşteri CRM Yönetimi</p>
          </div>
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === 'login') {
        ...
        setAuthMessage('Kayıt başarılı! E-postanıza gönderilen 6 haneli doğrulama kodunu girin.');
        setAuthMode('verify');
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
        <div className="bg-slate-800 border border-slate-700 rounded-2xl max-w-sm w-full p-6 shadow-2xl space-y-5">
          ...
          {authError && <div className="p-3 bg-red-900/50 border border-red-700 rounded-lg text-red-200 text-xs flex items-center gap-2"><AlertCircle className="w-4 h-4 shrink-0" /><span>{authError}</span></div>}
          {authMessage && <div className="p-3 bg-indigo-900/50 border border-indigo-700 rounded-lg text-indigo-200 text-xs flex items-center gap-2"><MailCheck className="w-4 h-4 shrink-0" /><span>{authMessage}</span></div>}
          ...
          <form onSubmit={handleAuth} className="space-y-3.5 text-xs">
            {authMode !== 'verify' ? (
              ...
              <div>
                <label className="block font-medium text-slate-300 mb-1">E-posta Doğrulama Kodu (OTP)</label>
                <input
                  type="text"
                  required
                  value={otpCode}
